...
namespace sp {
	...
	bool Response::remake_parameters(){

		TH1D ttemp = *(TH1D*)	_response_h.ProjectionY();	
		TH1D rtemp = *(TH1D*)	_response_h.ProjectionX();	

		//This will reconstruct the _true_param and _reco_param from the response matricies
		//_true_param->_hist = ttemp;	
		_reco_param->_hist = rtemp; 		

		//to refill response matrix (rather than hist)
		this->Finalize();
		return true;
	}

	bool Response::poisson_all(TRandom3 *rangen){
		//Need to poisson scale 


		this->remake_parameters();
		return true;
	}
