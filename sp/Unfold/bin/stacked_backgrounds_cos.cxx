...
int main(int argc, char** argv) {
	...
	std::string param = "CosTheta";
	std::vector<double> bins_reco = {-1,-0.8,-0.6,-0.4,-0.2,0,0.2,0.4,0.6,0.8,1};
	...
	for(int i=0; i< data->GetEntries();i++){
		data->GetEntry(i);
		h_data->Fill(d_CosTheta, d_Weight);
	}
	...
	THStack ths("ths","");
	TLegend tl(0.11,0.45,0.71,0.89);
	...
	excess_err.push_back(0);
	for(int i=0; i<bins_reco.size()-1;i++){
		summed_excess.push_back( summed_data.at(i)-summed_mc.at(i));
		excess_err.push_back(sqrt(summed_data.at(i) +pow(summed_mc_err.at(i),2) ) );
	}
	...
	ths.SetMaximum(std::max(ths.GetMaximum(), h_data->GetMaximum())*1.1);
	ths.SetMinimum(0.001);
...
//	p2->SetGrid();
	...
	excess->GetXaxis()->SetTitle("Cos(#theta)");
	excess->GetYaxis()->SetTitle("Excess");
	...
	TLine * l = new TLine(bins_reco.front(),0, 1,0);
	l->Draw();
	...
	c->SaveAs("cos_binning_scheme.pdf","pdf");
